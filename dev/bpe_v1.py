...
try:
    # prefer heap-optimized module if present
    from . import bpe_cpp_heap
    _HAS_BPE_HEAP = True
except Exception:
    bpe_cpp_heap = None
    _HAS_BPE_HEAP = False
...
def train_bpe(
    ...
    # 2. Pre-tokenization (same as before)
    ...
    if _HAS_BPE_HEAP:
        # Prepare vocab bytes list in order of indices
        max_vid = max(vocab.keys())
        vocab_list = [vocab[i] for i in range(max_vid + 1)]
        # call C++ heap trainer
        vocab_py, merges_py = bpe_cpp_heap.train_bpe_heap(ids, vocab_list, num_merges)
        # vocab_py is list of bytes objects
        # merges_py is list of (a,b) index tuples (these indices correspond to vocab_py)
        # convert to python structures
        vocab = {i: bytes(vocab_py[i]) for i in range(len(vocab_py))}
        merges = [ (int(a), int(b)) for (a,b) in merges_py ]
    else:
        # fallback python incremental (original behavior but using compute counts + apply)
